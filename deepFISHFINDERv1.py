...
with tf.variable_scope('FISHFINDER'):
    ...
    with tf.name_scope('C3_DIMS'):
        # shape of convolutional layer 2: [n_x, n_y, n_input_chan, n_output_chan]
        c3_shape = [3, 3, 8, 16]

    with tf.name_scope('C4_DIMS'):
        # shape of convolutional layer 2: [n_x, n_y, n_input_chan, n_output_chan]
        c4_shape = [3, 3, 16, 16]

    with tf.name_scope('C5_DIMS'):
        # shape of convolutional layer 2: [n_x, n_y, n_input_chan, n_output_chan]
        c5_shape = [3, 3, 16, 16]
    ...
    with tf.name_scope('C6_DIMS'):
        # shape of convolutional layer 2: [n_x, n_y, n_input_chan, n_output_chan]
        c6_shape = [3, 3, 16, 32]

    with tf.name_scope('C7_DIMS'):
        # shape of convolutional layer 2: [n_x, n_y, n_input_chan, n_output_chan]
        c7_shape = [3, 3, 32, 32]
    ...
    with tf.name_scope('conv_2'):
        ...
        c2 = cv.layer_conv(input=c1_dropout, weights=c2_weight)
    ...
    with tf.name_scope('pool_1'):
        c2_pooled = cv.layer_pool(c2_dropout, stride=pool1, type='max')

    with tf.name_scope('conv_3_params'):
        # weight and bias initialization for first convolutional layer
        c3_bias = cv.generate_bias(length=c3_shape[3])
        c3_weight = cv.generate_weights(c3_shape, type='p_uniform')

    with tf.name_scope('conv_3'):
        # first convolutional layer with relu and dropout, max pooling
        c3 = cv.layer_conv(input=c2_pooled, weights=c3_weight)
        c3_activate = cv.layer_activate(input=c3, bias=c3_bias, type='relu')
        c3_dropout = cv.layer_dropout(c3_activate, keep_probability=keep_prob)

    with tf.name_scope('conv_4_params'):
        # weight and bias initialization for first convolutional layer
        c4_bias = cv.generate_bias(length=c4_shape[3])
        c4_weight = cv.generate_weights(c4_shape, type='p_uniform')

    with tf.name_scope('conv_4'):
        # first convolutional layer with relu and dropout, max pooling
        c4 = cv.layer_conv(input=c3_dropout, weights=c4_weight)
        c4_activate = cv.layer_activate(input=c4, bias=c4_bias, type='relu')
        c4_dropout = cv.layer_dropout(c4_activate, keep_probability=keep_prob)

    with tf.name_scope('conv_5_params'):
        # weight and bias initialization for first convolutional layer
        c5_bias = cv.generate_bias(length=c5_shape[3])
        c5_weight = cv.generate_weights(c5_shape, type='p_uniform')

    with tf.name_scope('conv_5'):
        # first convolutional layer with relu and dropout, max pooling
        c5 = cv.layer_conv(input=c4_dropout, weights=c5_weight)
        c5_activate = cv.layer_activate(input=c5, bias=c5_bias, type='relu')
        c5_dropout = cv.layer_dropout(c5_activate, keep_probability=keep_prob)

    with tf.name_scope('pool_2'):
        c5_pooled = cv.layer_pool(c5_dropout, stride=pool2, type='max')

    with tf.name_scope('conv_6_params'):
        # weight and bias initialization for first convolutional layer
        c6_bias = cv.generate_bias(length=c6_shape[3])
        c6_weight = cv.generate_weights(c6_shape, type='p_uniform')

    with tf.name_scope('conv_6'):
        # first convolutional layer with relu and dropout, max pooling
        c6 = cv.layer_conv(input=c5_pooled, weights=c6_weight)
        c6_activate = cv.layer_activate(input=c6, bias=c6_bias, type='relu')
        c6_dropout = cv.layer_dropout(c6_activate, keep_probability=keep_prob)

    with tf.name_scope('conv_7_params'):
        # weight and bias initialization for first convolutional layer
        c7_bias = cv.generate_bias(length=c7_shape[3])
        c7_weight = cv.generate_weights(c7_shape, type='p_uniform')

    with tf.name_scope('conv_7'):
        # first convolutional layer with relu and dropout, max pooling
        c7 = cv.layer_conv(input=c6_dropout, weights=c7_weight)
        c7_activate = cv.layer_activate(input=c7, bias=c7_bias, type='relu')
        c7_dropout = cv.layer_dropout(c7_activate, keep_probability=keep_prob)

    with tf.name_scope('flatten'):
        # flatten output of convolutional network
        flattened, total_params = cv.layer_flatten(c7_dropout)
...
if __name__ == "__main__":
    ...
    test = 'NO'
    ff_test = 'NO'
    save_dir = 'C:/Users/Donald/Desktop/kaggle_fish_competition/fishes/code_revamp/savestate_dfishFinder/'  # directory to save weights in
    train_imdir = 'C:/Users/Donald/Desktop/kaggle_fish_competition/fishes/data/train_ds/'  # directory with all training image subdirectories
    test_imdir = 'C:/Users/Donald/Desktop/kaggle_fish_competition/fishes/data/test_ds/'
    img_dims = (100, 100, 1)
    ...
    if train == 'YES':
        ...
        t1, t2, t3 = train_network(num_iterations=100, multiplier=100, imagedirs=train_dirs_dict, classes=class_dict,
