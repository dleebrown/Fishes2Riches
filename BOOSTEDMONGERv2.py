...
def train_booster(xgb_params, num_rebuilds, num_estimators, image_dims, classes, train_imagedirs, train_infodict,
    ...
    for i in range(num_rebuilds):  # num rebuilds is the number of redraws of the training set
        ...
        if i == 0:  # initializes the trees and performs the first training loop
            xgb_model = xgb.train(xgb_params, training_data, num_estimators, evals=watchlist, early_stopping_rounds=100)
            ...
            xgb_model = xgb.train(xgb_params, training_data, num_estimators, evals=watchlist,
                                  xgb_model=xgb_model, early_stopping_rounds=100)
...
def feed_forward_fishbooster(image_array, saved_state, n_classes=7):
...
if __name__ == "__main__":
    ...
    train_bal_dict = {'ALB': 1500, 'BET': 1500, 'DOL': 1500,
                      'LAG': 1500, 'OTHER': 1500, 'SHARK': 1500,
                      'YFT': 1500}
    ...
    test_bal_dict = {'ALB': 30, 'BET': 10, 'DOL': 5,
                      ...
                      'YFT': 25}
    img_dims = (100, 100, 1)
    model_name = 'trained_boostmong_model'
    ...
    hyperparams = {'silent': 1, 'nthread': 4, 'objective': 'multi:softprob',
                     'eval_metric': 'mlogloss', 'num_class':7}
    tuned_params = {'max_depth': 7, 'min_child_weight': 0.3, 'gamma': 0.1, 'eta': 0.005, 'subsample': 0.9,
                    'colsample_bytree': 0.9, 'scale_pos_weight': 1.0}
    hyperparams.update(tuned_params)

    t1, t2, t3 = train_booster(xgb_params=hyperparams, num_rebuilds=1, num_estimators=5000, image_dims=img_dims,
    ...
    test_outputs = feed_forward_fishbooster(test_input_array, 'trained_boostmong_model')
    ...
    for i in range(total_class):
        if classdiff[i] == 0:
            correct_class += 1
    print(test_outputs[10:15, 3:])
    print(test_class[10:15])
