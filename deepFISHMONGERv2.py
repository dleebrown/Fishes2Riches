...
def batch_balance(fish_info_dict, balance_dict):
    keys = list(fish_info_dict.keys())
    batch_out = []
    for fish in keys:
...
with tf.variable_scope('FISHMONGER'):
    ...
    with tf.name_scope('conv_2'):
        ...
        c2 = cv.layer_conv(input=c1_activate, weights=c2_weight)
        c2_activate = cv.layer_activate(input=c2, bias=c2_bias, type='relu')

    with tf.name_scope('pool_1'):
        c2_pooled = cv.layer_pool(c2_activate, stride=pool1, type='max')
    ...
    with tf.name_scope('pool_2'):
        c3_pooled = cv.layer_pool(c3_activate, stride=pool2, type='max')

    with tf.name_scope('flatten'):
        # flatten output of convolutional network
        flattened, total_params = cv.layer_flatten(c3_pooled)
    ...
    with tf.name_scope('optimize'):
        ...
        optimize = cv.network_optimize(cost, learn_rate=1e-5, optimizer='adam')
...
def train_network(num_iterations, multiplier, imagedirs, classes, infodict, balancedict, image_dims, batch_size,
    ...
    for i in range(num_loops):
        ...
        for e in range(multiplier):
            single_batch = batch_balance(fishinfo, balancedict)
        ...
        for itera in range(multiplier):
            ...
            input_dict = {image_data: input_px, true_class: input_class, im_x: image_dims[0], im_y: image_dims[1],
                        im_z: image_dims[2], num_classes: num_class, keep_prob: 0.8}
...
def test_network_monger(imagedirs, classes, infodict, balancedict, image_dims):
    ...
    fishinfo = build_fish_info_dict(infodict)
    start_time2 = time.time()
    input_queue = batch_balance(fishinfo, balancedict)
    queue_indices = list(range(len(input_queue)))  # this is just a hacky thing to avoid recoding train_batch_create_mp
    ...
    for i in range(3):
        ...
        file = open('fm_out'+names[i]+'.txt', 'w')
        for line in lists[i]:
            file.write(str(line)+' \n')
        file.close()
...
def feed_forward_fishmonger(image_array, image_dims, n_classes, saved_state):
    ...
    session = tf.Session()
    saver_fm = tf.train.Saver([var for var in tf.global_variables() if 'FISHMONGER' in var.name])
    saver_fm.restore(session, saved_state)
...
if __name__ == "__main__":
    ...
    test = 'YES'
    ff_test = 'NO'
    save_dir = '/home/donald/Desktop/kaggle_fish_competition/fishes/code_revamp/savestate_dfishMonger/'  # directory to save weights in
    train_imdir = '/home/donald/Desktop/kaggle_fish_competition/fishes/data/train_ds/'  # directory with all training image subdirectories
    test_imdir = '/home/donald/Desktop/kaggle_fish_competition/fishes/data/test_ds/'
    img_dims = (100, 100, 3)
    class_dict = {'ALB': 0, 'BET': 1, 'DOL': 2, 'LAG': 3, 'OTHER': 4, 'SHARK': 5, 'YFT': 6}
    ...
    train_bal_dict = {'ALB': 30, 'BET': 10, 'DOL': 5,
    ...
    if train == 'YES':
        ...
        #toggle gpu usage on or off
        #config = tf.ConfigProto(device_count={'GPU':0})
        #session = tf.Session(config=config)
        session = tf.Session()
        session.run(tf.global_variables_initializer())
        saver = tf.train.Saver()
        t1, t2, t3 = train_network(num_iterations=100, multiplier=25, imagedirs=train_dirs_dict, classes=class_dict,
    ...
    if continue_train == 'YES':
        ...
        t1, t2, t3 = train_network(num_iterations=100, multiplier=50, imagedirs=train_dirs_dict, classes=class_dict,
                                   infodict=train_key_dict, balancedict=train_bal_dict, image_dims=img_dims,
                                   batch_size=100, save_path=save_dir)
        save_path = saver.save(session, save_dir + 'save.ckpt')
    ...
    if test == 'YES':
        ...
        t1, t2, acc = test_network_monger(test_dirs_dict, class_dict, test_key_dict, test_bal_dict, img_dims)
        session.close()
        print('total run time: ' + str(round(t1, 2)))
        print('tensorflow time: ' + str(round(t2, 2)))
